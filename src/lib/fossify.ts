...
export const Frontends:{[key:string]:any} = require("../../Frontends.json")
export const FossifySettings:{[key:string]:any} = require("../../Fossify.json")

export default function fossify(str:string,proxy?:boolean) {
    let morty = ""
    if (proxy && typeof FossifySettings.morty == "string") {
        morty = FossifySettings.morty
    }
    ...
    if (_links.length > 0) {
        ...
        links.forEach((url) => {
            ...
            if (!foundFE && morty) {
                transformedURLs.push({originalURL:url.domain+"/"+url.fullpath,newURL:`${morty}${url.domain}/${url.path}/post?mortyurl=${encodeURI(`https://${url.domain}/${url.path}`)}`})
            }
        })
